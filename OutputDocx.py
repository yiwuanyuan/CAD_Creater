...
def OutputDocx(info,address,isNeedML):
   ...
   # 新设置一种名为"tbhead"的新style,设置字体大小，字体样式
    ...
    y = 0
    for list in info:
        item_material = '材料牌号:' + str(list['material'])+' 厚度:'+str(list['thickness'])
        # print(item_material)
    ...
    #签名栏重写
    table_sign = doc.add_table(rows=2, cols=6, style='Table Grid')
    ...
    table_sign.cell(0, 4).paragraphs[0].add_run('审 核:').style = sign_style
    ...
    table_sign.cell(1, 4).paragraphs[0].add_run('调度员:').style = sign_style
    table_sign.cell(1, 4).vertical_alignment = WD_ALIGN_VERTICAL.CENTER
    table_sign.cell(1, 4).paragraphs[0].paragraph_format.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    table_sign.cell(1, 0).width = Mm(30)
    table_sign.cell(1, 1).width = Mm(50)
    table_sign.cell(1, 2).width = Mm(30)
    table_sign.cell(1, 3).width = Mm(50)
    table_sign.cell(1, 4).width = Mm(30)
    table_sign.cell(1, 5).width = Mm(50)

    table_sign.rows[0].height = Mm(12)
    table_sign.rows[1].height = Mm(12)
